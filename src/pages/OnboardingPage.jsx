  ...
  return (
    <div className="fixed inset-0 overflow-hidden select-none" {...swipe}>
      ...
      <div className="absolute inset-0 transition-all duration-700"
        style={{ background: `linear-gradient(160deg, ${slide.bg[0]} 0%, ${slide.bg[1]} 50%, ${slide.bg[2]} 100%)` }}/>
      ...
      <div className="relative h-full flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 pt-14 pb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-black text-lg" style={{ fontFamily: 'Georgia,serif' }}>MANG</span>
          </div>
        ...
        {/* Illustration */}
        <div className={clsx('flex justify-center mt-8 transition-all duration-320',
          ...
          <div className="relative">
            <div className="w-44 h-44 rounded-[3rem] flex items-center justify-center"
              style={{
                background: `linear-gradient(135deg, ${slide.accent}18, ${slide.accent}08)`,
                border: `1.5px solid ${slide.accent}30`,
                backdropFilter: 'blur(20px)',
                boxShadow: `0 30px 80px rgba(0,0,0,0.5), inset 0 1px 0 ${slide.accent}20`,
              }}>
              <span style={{ fontSize: '6rem', filter: 'drop-shadow(0 8px 24px rgba(0,0,0,0.5))' }}>
                {slide.emoji}
              </span>
            </div>
            {/* Stat badge */}
            <div className="absolute -bottom-4 -right-4 px-4 py-2 rounded-2xl text-center"
              style={{ background: slide.accent, boxShadow: `0 8px 24px ${slide.accent}60` }}>
              <p className="text-black font-black text-xl leading-none">{slide.stat.v}</p>
              <p className="text-black/60 text-[10px] font-bold mt-0.5">{slide.stat.l}</p>
            </div>
            {/* Numéro */}
            <div className="absolute -top-3 -left-3 w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm"
        ...
        {/* Texte */}
        <div className={clsx('px-7 mt-10 flex-1 transition-all duration-320',
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5"
            ...
            <span className="text-[11px] font-bold tracking-wider uppercase" style={{ color: slide.accent }}>
              {slide.tag}
            </span>
          </div>
          <h2 className="text-white font-black leading-[1.08] mb-4"
            style={{ fontSize: '2.4rem', fontFamily: 'Georgia,serif', whiteSpace: 'pre-line' }}>
            {slide.title}
          </h2>
          <p className="text-white/65 text-sm leading-relaxed">{slide.sub}</p>
        </div>
          ...
          <div className="flex items-center gap-2 mb-7">
            {SLIDES.map((_, i) => (
              <button key={i} onClick={() => i !== idx && go(i, i > idx ? 1 : -1)}
                ...
                style={{ width: i === idx ? 28 : 7, height: 7, background: i === idx ? slide.accent : 'rgba(255,255,255,0.2)' }}/>
            ))}
          </div>
          <button onClick={next}
            className="w-full py-4 rounded-2xl font-black text-base tracking-wide active:scale-95 transition-all"
            style={{ background: `linear-gradient(135deg, ${slide.accent}, ${slide.accent}bb)`, color: '#000', boxShadow: `0 12px 40px ${slide.accent}45` }}>
...
function LandingScreen() {
  ...
  return (
    <div className="fixed inset-0 overflow-y-auto"
      style={{ background: 'linear-gradient(160deg, #040d07 0%, #0a2514 35%, #071a0d 100%)' }}>

      {/* Déco fond */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 inset-x-0 h-[45vh] opacity-20"
          style={{ background: 'radial-gradient(ellipse at 50% -10%, #2ECC71, transparent)' }}/>
        <div className="absolute bottom-0 inset-x-0 h-48 opacity-12"
          style={{ background: 'radial-gradient(ellipse at 50% 110%, #f5a623, transparent)' }}/>
        <div className="absolute inset-0 opacity-[0.02]"
          style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,.4) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.4) 1px, transparent 1px)', backgroundSize: '32px 32px' }}/>
      </div>

      <div className="relative min-h-full flex flex-col pb-12">
        {/* Hero */}
        <div className="text-center px-6 pt-14 pb-6">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-7"
            ...
            <span className="text-green-400 text-xs font-bold tracking-widest uppercase">🌍 Marketplace Agricole Bénin</span>
          </div>

          <div className="flex justify-center mb-6">
            <div className="w-24 h-24 rounded-[1.8rem] flex items-center justify-center"
              style={{ background: 'linear-gradient(135deg, #2ECC71, #1B8F3A)', boxShadow: '0 0 100px rgba(46,204,113,0.3), 0 20px 60px rgba(0,0,0,0.5)' }}>
              <span style={{ fontSize: '3.5rem' }}>🌿</span>
            </div>
          </div>

          <h1 className="font-black text-white mb-2"
            style={{ fontSize: '3.5rem', fontFamily: 'Georgia,serif', lineHeight: 1, letterSpacing: '-2px' }}>
            <span style={{ color: '#2ECC71' }}>M</span>
            <span style={{ color: '#f5a623' }}>A</span>
            <span style={{ color: '#2ECC71' }}>N</span>
            <span style={{ color: '#f5a623' }}>G</span>
          </h1>
          <p className="text-white/35 text-xs tracking-[0.3em] uppercase mb-6 font-semibold">
          ...
          <p className="text-white/75 text-base leading-relaxed max-w-xs mx-auto">
            ...
            <strong className="text-green-400">producteurs</strong> et{' '}
            <strong className="text-yellow-400">acheteurs</strong>{' '}
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <div key={i} className="flex flex-col items-center py-4 rounded-2xl"
                ...
                <p className="text-white/30 text-[9px] font-bold uppercase tracking-wider mt-1">{s.l}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Features */}
        <div className="px-5 mb-7">
          <p className="text-white/25 text-[10px] font-bold tracking-widest uppercase mb-4 text-center">Tout ce dont vous avez besoin</p>
        ...
        <div className="px-5 mb-8">
          <div className="p-5 rounded-2xl relative overflow-hidden"
            style={{ background: 'linear-gradient(135deg, rgba(46,204,113,0.1), rgba(245,166,35,0.06))', border: '1px solid rgba(46,204,113,0.18)' }}>
            <div className="text-4xl font-black opacity-15 absolute top-2 left-4 text-green-400">"</div>
            <p className="text-white/80 text-sm leading-relaxed pl-4 mb-4 italic min-h-[3.5rem]">{t.text}</p>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-xl flex-shrink-0"
                style={{ background: 'rgba(46,204,113,0.15)', border: '1px solid rgba(46,204,113,0.2)' }}>
                {t.avatar}
              </div>
              <div className="flex-1">
                <p className="text-green-400 font-bold text-sm">{t.name}</p>
                <p className="text-white/35 text-xs">{t.role}</p>
              </div>
              <div className="text-yellow-400 text-xs">{'⭐'.repeat(t.stars)}</div>
            </div>
            <div className="flex justify-center gap-1.5 mt-4">
              {TESTIMONIALS.map((_, i) => (
                <div key={i} className="rounded-full transition-all duration-300"
                  style={{ width: i === tIdx ? 20 : 6, height: 6, background: i === tIdx ? '#2ECC71' : 'rgba(255,255,255,0.15)' }}/>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="px-5">
          <button onClick={() => navigate('/inscription')}
            className="w-full py-4 rounded-2xl font-black text-base tracking-wide mb-3 active:scale-95 transition-all text-white"
            style={{ background: 'linear-gradient(135deg, #2ECC71, #1B8F3A)', boxShadow: '0 12px 50px rgba(46,204,113,0.4)' }}>
          ...
          <div className="flex items-center justify-center gap-5 mt-6">
            {['🔒 Sécurisé', '🇧🇯 Made in Bénin', '⭐ Gratuit'].map((b, i) => (
              <span key={i} className="text-white/20 text-[10px] font-semibold">{b}</span>
            ))}
          </div>
